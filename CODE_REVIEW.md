...
## 24. Дополнительные рекомендации (раунд 2)

После повторного прохода появились наблюдения, которые не вошли в первый разбор. Сгруппированы по темам.

### 24.1 UI (`ui.py`)

- **Tkinter/ttk морально устарел** для долгоиграющего «панель управления + лента логов» UI. Темизация через `Style.configure(...)` сложно поддерживать, нет hover-анимаций, шрифт-стек прибит к Windows (`Segoe UI`, `Consolas`). Лучше переехать на **CustomTkinter** (минимальное изменение API, тёмная тема из коробки) либо **PySide6** (если планируется дашборд с графиками атак).
- **UI блокирует поток при `controller.stop()`**: `process.wait(timeout=8)` выполняется прямо в Tk-цикле — окно подвисает на 8 секунд при «зависшем» процессе. Должен выполняться в `threading.Thread` или асинхронно (`after_idle` + poll).
- **`read_new_log` читает файл синхронно каждую секунду** при больших chunk'ах (десятки КБ): на slow-FS это микро-фриз. Альтернатива — таскать tail в отдельном потоке через `queue.Queue`, UI только опустошает очередь.
- **Stdout процесса бота сваливается в `logs/ui-process.log`** и одновременно loguru пишет тот же поток в `logs/bot.log` — дубль. Достаточно `subprocess.DEVNULL` для stdout/stderr.
- **Состояние мёртвого процесса не отображается**: если процесс упал сам, кнопка «Start» всё ещё помечена «Running» до следующего `is_running()` опроса. Нужно периодически опрашивать `poll()` и менять статус на «Crashed».
- **Захардкоженный список аккаунтов** (`proxima/yung_proxima/old_proxima`) — должен браться из `BotConfig.accounts`, чтобы UI не ломался при переименовании.
- **Нет горячих клавиш и индикатора PID**, нет окна логов в отдельной вкладке (если их станет несколько типов: bot/actions/calibration).

### 24.2 ADB / производительность

- **`adb.exe` стартует на каждый вызов `_base_cmd()`** даже после persistent-shell-рефакторинга — кеш `Path(self.adb_path).exists()` имеет смысл выполнять один раз в `__init__` (или ленивая проверка с invalidation после ошибок).
- **`tap_many_percent` создаёт `ThreadPoolExecutor` на каждый вызов** — для серии burst-тапов это до 8 коротко живущих пулов в секунду. Лучше держать долгоживущий `_executor` на устройстве (lazy init, закрывается в `close()`).
- **`screenshot()` ретраит 3 раза с `sleep(0.5)` без exponential backoff**. На фатальной просадке эмулятора получаем фиксированные 1.5 c, после чего исключение наверх — recovery всё равно перезапустит ADB. Имеет смысл: 1 быстрый retry → сразу эскалация в recovery, без бесполезных задержек.
- **Скриншот через `screencap -p` декодит PNG**. На LDPlayer есть `screencap` без `-p` (raw RGBA) — в 1.5–2× быстрее декодирование (`np.frombuffer` без `cv2.imdecode`). Минус — нужна знание ширины/высоты заранее, но они уже кешируются в `_screen_size`.
- **`ctrl_mouse_wheel_zoom_out` / `press_emulator_key` используют Win32 `keybd_event`** — это глобальная клавиатура, если игрок переключится на другое окно во время бота — кнопки уйдут не туда. Хотя бы залогировать имя foreground-window перед посылкой, идеально — `SendMessage` напрямую в hwnd.

### 24.3 Vision / OCR

- **`_template_cache` хранит и BGR-, и gray-копию шаблона** через два словаря. Доступ к BGR-копии нужен только для редкого debug-overlay — gray-only хранилище экономит память (×3 на больших шаблонах).
- **OCR на `state_ocr_*_area` запускается до 3 раз в одном `_detect_state_once`** — это самая тяжёлая операция в цикле детекции. Если хотя бы один из трёх area совпадает с другим — кадр OCR'ится дважды. Кэш по `(area, image_id)` внутри одного вызова решает.
- **`reader.readtext(image, paragraph=True)`** разбивает на абзацы, что замедляет распознавание короткой UI-кнопки. Для «Find a Match»/«Okay» хватит `paragraph=False`.
- **`_detect_boundary_candidates`** делает три `inRange` и два `morphologyEx` на каждом кадре — на 1080p это ~30–50 мс. Маски можно собрать в одну операцию через `np.where` или предвычислить HSV-диапазоны.
- **Нет защиты от устаревшего `_stable_state`**: если игра «зависла» в одном экране на 10 мин, бот всё равно считает state стабильным. Добавить TTL: после N секунд без подтверждений сбрасывать в UNKNOWN и форсить полную детекцию.

### 24.4 Конфигурация

- **`config.example.json` (14 КБ)** держится рядом с `BotConfig` без проверки актуальности. CI/lint должен прогонять `_config_from_dict(json.load(example))` и валить если поля разъехались.
- **Нет миграций конфига**: при добавлении нового поля старые `config.json` пользователей сломаются (нет default-фолбэка). Решение — обернуть `dataclass` в `field(default_factory=...)` для всех необязательных или поддержать `extra_fields="ignore"` режим.
- **Логические флаги типа `okay_button_enabled`, `builder_first_slot_retap_enabled`** не покрыты тестами. Хотя бы smoke-тест «загружается `config.example.json` → `validate_config` без ошибок».
- **Отсутствуют schema-файлы** (JSON Schema / pydantic). Pydantic v2 даст бесплатные сообщения об ошибках, миграции и автодокументацию.

### 24.5 Логирование и observability

- **`actions.log` пишется в один файл без ротации** — за неделю 24/7 это многие сотни МБ. Loguru поддерживает `rotation="50 MB", retention="7 days"` — добавить.
- **Нет structured-логов**: всё текстом. Метрики (атак за час, средний loot, retry-count) приходится грести regex'ом по логу. Решение — параллельно писать `logs/metrics.jsonl` (по одной строке на цикл).
- **Loguru-handler с лямбдой через `print`** (см. `main.py`) перехватывает stdout и ломает прогресс-бары/цвет в Windows-консоли. `sys.stderr` корректнее.

### 24.6 Тестируемость / CI

- **Тестов на `vision`/`battle_flow` нет**. Хотя бы один snapshot-тест: подложить фиксированный кадр PNG в `_find_template_in_image` и проверить координату/скор. Без этого любой рефакторинг template-логики катится «втёмную».
- **Нет CI-пайплайна**: `pytest` + `ruff` + `mypy --strict coc_bot/` дадут защиту от опечаток и регрессий конфига.
- **`AdbDevice` сложно мокать** — он принимает `adb_path: str` и проверяет существование файла. Удобнее принимать абстракцию `AdbTransport` (Protocol) с методами `run/run_bytes/screenshot`, тогда фейк в тестах — одна строка.

### 24.7 Recovery и устойчивость

- **`recover(exc)` в `main.py` нет адаптивного back-off**: одна и та же ошибка зацикливается → много логов и быстрый износ диска. Стандартный паттерн: при подряд N (≥3) фейлах — увеличивать паузу экспоненциально и слать Telegram-алерт.
- **Watchdog по времени отсутствует**: если battle_flow застрял в OCR-чтении 30 секунд, нет внешнего таймера, который зарежет цикл. Решение — фоновый поток-watchdog, который вызывает `device.kill_server()` при превышении лимита одной итерации.
- **Health-чек выполняется только до цикла**. После recovery нужен health.check_after_recovery() — иначе мы можем войти в `BattleFlow` на ещё не загруженном экране.

### 24.8 Безопасность / приватность

- **`.env` отсутствует в `.gitignore`** (требует проверки) — Telegram-токен может утечь.
- **Скриншоты в `logs/screenshots/`** могут содержать никнеймы/клан-тэги. При публикации логов в issue/баг-репорт это PII. Добавить опцию «blur top-bar» при сохранении.
- **`subprocess.Popen` в UI без `shell=False` явно** — норм, но стоит включить `creationflags=CREATE_NEW_PROCESS_GROUP` на Windows, иначе Ctrl+C в родительском окне убивает и UI, и бота.

### 24.9 Документация

- **Нет ADR / architecture.md**: при появлении нового разработчика «почему OCR + шаблоны, а не только шаблоны?» неоткуда узнать.
- **README не описывает требования к LDPlayer** (версия, разрешение, ADB-флаг включён). Это первая боль новых пользователей.
- **`AGENTS.md` есть — хорошо**, но его стоит дополнить разделом «как добавить новый шаблон UI-кнопки» (шаги: PNG в `assets/`, поле в `BotConfig`, тест-snapshot, конфиг).

---

## 25. Обновлённый TOP-список (с учётом раунда 2)

Сразу делаем (быстро и больно решает):
1. Persistent ADB shell.
2. Single-screenshot detection per step.
3. Раздельный `actions.log` с ротацией (loguru rotation).
4. UI: миграция на CustomTkinter + асинхронный stop + queue-based log tail.
5. Watchdog-таймер на итерацию + adaptive backoff в recovery + Telegram-алерт.

Среднее ближайшее:
6. JSONL-метрики и health-after-recovery.
7. Декомпозиция `BotConfig`, schema-валидация через pydantic v2.
8. `screencap` без `-p` (raw RGBA) для ускорения декодинга.
9. Pinning версий + минимальный CI (ruff + pytest + проверка `config.example.json`).
10. Snapshot-тесты для vision.

Долгое (рефакторинг):
11. Замена OCR на шаблоны там, где можно.
12. FSM-обёртка над флоу.
13. Дашборд (если PySide6 / web-UI).
